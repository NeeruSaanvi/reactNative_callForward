...
class MCP extends Component {
  ...
  submit = async () => {
    ...
    if (this.state.sfed === "") {
      this.setState({message: "Please input date and time!"});
      return false;
    }

    let message = multi_conversion(this.props.somos.id, this.props.somos.selectRo, fix_num(this.state.nums), this.state.date, this.state.template);
    console.log(message);
    ...
    if (res.ok && res.data && res.data.message) {
      ...
      if (errors && errors.length) {
        for (let i = 0; i < errors.length; i++) {
          ...
          err_message += error[2] + ":  " + error_customer_record_message(parseInt(error[0]).toString())+ "\n";
        }
        this.setState({message: err_message});
        return false;
      } else {
        this.setState({message: "Multi Conversion Succeeded"})
      }
