...
import {verb, timeout, customer_number, customer_selection, padToCad, deletePad, transferPad} from "../../../service/customer";
import {error_customer_record_message, error_number_query,error_customer_query_message, error_template_query_message} from "../../../service/error_message";
import {state_value, validLAD, temp, approval_type} from "../../../service/template";
import {cleanLocalStorage, fixed_date, push_cpr} from "../../../utils";
import {create_pointer_record, disconnect_pointer, resend_pointer, update_pointer_record} from "../../../service/pointer";
...
class PointerData extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false, activeTab: '1', num: "", sfed: '', ed: "", et: '', ro: '', approval: '', last: '', prev_user: '', by: '', action: '', so: '', sf: '', ddt: '', customer: '',
      agent: '', telco: '', hold: '', directory: '', dau: '', dat: '', list_name: '', list_address: '', ncon: '', ctel: '', notes: '', abn: '', rao: '', ic: '', end: '',
      lns: '', city: '', uts: '', line: '', lsis: '', lso: '', fso: '', stn: '', sfg: '', hml: '', referral: 'N', message: '', isDate: false,
      retrieve: false, update: false, copy: true, transfer: true, delete: true, cpr: false, lad: false, rec: false, disable: false, large: false, ed_origin: "",
      et_origin: '', toggle_delete: false, dcsn: "", template: "", ed_cpr: "", et_cpr: "", tran: false, ed_tran: '', et_tran: '',copy_num: '', copy_cr: false,
      copy_lad: false, copy_cpr: false,isLad: false, isCpr: false, iec_array: [], iac_array: [], dates:[], is_cpr: false, is_lad: false,
  ...
  componentDidMount = () => {
    ...
    if (num) {
      if (sfed) {
        dates = dates.split(",");
        this.setState({num: num, sfed: sfed, dates: dates});
        sfed = sfed.split(" ");
        this.get_pointer_record(num, sfed[0], sfed[1]);
      } else {
  ...
  get_pointer_record = async (num, ed, et) => {
    ...
    if (res.ok && res.data && res.data.message) {
      ...
      if (message.value("TMPLTPTR")[0]) {
        this.setState({
          ...
          template: message.value("TMPLTPTR")[0],
          ...
          message: "PAD retrieve successfully",
          copy: false,
          transfer: false,
          delete: false,
          disable: true
  ...
  retrieve_number = async (num, sfed) => {
    this.setState({ro: '', ddt: '', telco: '', so: '', template: '', ncon: '', ctel: '', number: '', lns: '', end: '', referral: "N", message: ''});
        ...
        if (params.length === 0) {
          ...
          if (res.ok && res.data && res.data.message) {
            ...
            if (message.value('STAT')[0].trim() !== "SPARE") {
              ...
              message.value("ERR")[0] ? this.setState({message: error_customer_record_message(message.value("ERR")[0])}) : this.setState({message: "To create a new 'PAD', please enter the EFF Date/Time on PAD!"});
          ...
          this.get_pointer_record(fix_num(num), params[0].ED, params[0].ET)
        } else {
          ...
          this.get_pointer_record(fix_num(num), ed, et);
        }
      }
    } else {
      let date = this.state.sfed;
      date = date.split(" ");
      this.get_pointer_record(fix_num(num), date[0], date[1]);
    }
  };
  ...
  update_pad = async () => {
    ...
    if (this.state.action === "C") {
      message = update_pointer_record(this.props.somos.id, this.props.somos.selectRo, "C", fix_num(this.state.num), this.state.sfed, this.state.abn,
        this.state.dau, this.state.dat, this.state.ddt, this.state.hold, this.state.directory, this.state.rao, this.state.so, this.state.sf, this.state.notes, this.state.agent,
        this.state.telco, this.state.customer, this.state.ro, this.state.list_address, this.state.ic, this.state.ncon, this.state.ctel, this.state.list_name, fix_num(this.state.tel),
        this.state.city, this.state.fso, this.state.lns, this.state.hml, this.state.lsis, this.state.lso, this.state.sfg, this.state.stn, this.state.uts, lads, types, this.state.gridData,
        this.state.template, this.state.copy_cpr, this.state.copy_cr, this.state.copy_lad);
    } else if (this.state.action === "N") {
      if (this.state.so === "") {this.setState({message: "Please input service order"});return false;}
      if (this.state.sfed === "") {this.setState({message: "To create a new 'PAD', please enter the EFF Date/Time on PAD!"});return false;}
      if (this.state.template === "") {this.setState({message: "Please input Template"});return false;}
      ...
      message = create_pointer_record(this.props.somos.id, this.props.somos.selectRo, this.state.action, fix_num(this.state.num), this.state.sfed, this.state.abn, this.state.dau, this.state.dat, this.state.ddt, this.state.hold,
        this.state.directory, this.state.rao, this.state.so, this.state.sf, this.state.notes, this.state.agent, this.state.telco, this.state.customer, this.state.ro,
        this.state.list_address, this.state.ic, this.state.ncon.trim(), this.state.ctel, this.state.list_name, fix_num(this.state.tel), this.state.city, this.state.fso,
        this.state.lns, this.state.hml, this.state.lsis, this.state.lso, this.state.sfg, this.state.stn, this.state.uts, lads, types, this.state.gridData, this.state.template);
    } else if (this.state.action === "D") {
      if (this.state.end === "") {
        this.setState({message: "Please input End Intercept!"});
        return false;
      }
      message = disconnect_pointer(this.props.somos.id, this.props.somos.selectRo, this.state.action, fix_num(this.state.num), this.state.sfed, this.state.referral, this.state.end);
    } else if (this.state.action === "R") {
      message = resend_pointer(this.props.somos.id, this.props.somos.selectRo, this.state.action, fix_num(this.state.num), this.state.sfed);
    }
    console.log(message);
    ...
    if (res.ok && res.data && res.data.message) {
      ...
      if (this.state.action === "N") {
        if (message.value("ERR")[0]) {
          ...
          dates.push(date);
          this.setState({message: "PAD create successfully!", disable: true, dates: dates, sfed: date,})
        }
        ...
        if (message.value("ERR")[0]) {
          ...
          date = message.value("ED")[0] + " " + message.value("ET")[0] + " " + state;
          dates.push(date);
          this.setState({message: "PAD update successfully!", disable: true, dates: dates, sfed: date})
        }
      } else if (this.state.action === "D") {
        message.value("ERR")[0] ? this.setState({message: error_customer_record_message(message.value("ERR")[0]), sfed: ''}) : this.setState({message: "PAD disconnect successfully", sfed:''});
      } else if (this.state.action === "R") {
        message.value("ERR")[0] ? this.setState({message: error_customer_record_message(message.value("ERR")[0])}) : this.setState({message: "PAD resend successfully"});
      }
    }
  };
  ...
  copy_pointer = () => {
    ...
    this.setState({dates: dates, sfed: date, large: false, action: "C",disable: false, update: false,message:"PAD copy successfully!"});
  ...
  delete_pointer = async () => {
    this.setState({toggle_delete: false});
    let date = fixed_date(this.state.delete_sfed, this.state.delete_now);
    let message = deletePad(this.props.somos.id, this.props.somos.selectRo, date, fix_num(this.state.copy_num), this.state.template);
    console.log(message);
    let res = await this.props.callApi2(RestApi.sendRequestNew, {'mod': "CRC", 'message': message, 'timeout': timeout});
    if (res.ok && res.data && res.data.message) {
      console.log(res.data.message);
      let message = new MgiMessage(res.data.message);
      message.value("ERR")[0] ? this.setState({message: error_customer_record_message(message.value("ERR")[0])}): this.setState({message: "PAD delete successfully"});
      this.setState({disable: true});
    }
  };

  convertPadToCad = async () => {
    this.setState({cpr: false});
    let date = fixed_date(this.state.rec_sfed, this.state.rec_now);
    let message = padToCad(this.props.somos.id, this.props.somos.selectRo, date, fix_num(this.state.num), this.state.sfed);
    console.log(message);
    let res = await this.props.callApi2(RestApi.sendRequestNew, {'mod': "CRC", 'message': message, 'timeout': timeout});
    if (res.ok && res.data && res.data.message) {
      console.log(res.data.message);
  ...
  transfer = async () => {
    ...
    let message = transferPad(this.props.somos.id, this.props.somos.selectRo, date, fix_num(this.state.copy_num), this.state.template, this.state.ed_origin, this.state.et_origin);
    console.log(message);
    let res = await this.props.callApi2(RestApi.sendRequestNew, {'mod': "CRC", 'message': message, 'timeout': timeout});
    if (res.ok && res.data && res.data.message) {
      ...
      if (message.value("ERR")[0]) {
        ...
        this.setState({
          message: "Successfully transfer PAD",
  ...
  toogleDelete = () => {this.setState({toggle_delete: !this.state.toggle_delete})};
  ...
  render() {
    return (
      <div className="animated fadeIn mt-1">
          ...
          <Col xs="12">
            <Card>
              <div className="mt-3 mb-1 ml-4 mr-4" style={{backgroundColor: '#dfe1e3'}}>
                <Row className="mt-2 ml-4 mr-4 pb-2">
                  <Col xs="12" md="6" className="row">
                    <Label className="col-5 font-weight-bold">Toll Free Number * :</Label>
                    <Input className="col-4 form-control-sm" type="text" name="num" id="num" onChange={(ev) => this.handle(ev)} value={this.state.num}/>
                  </Col>
                  <Col xs="12" md="6" className="row">
                    <Label className="col-6 font-weight-bold">Eff.Date/Time/Status:</Label>
                      ...
                      <select className="col-4 form-control-sm" name="sfed" id="sfed" onChange={(ev) => this.handle(ev)} value={this.state.sfed}>
                        <>
                          {this.state.dates.map(value => {
                            return value.includes("ACTIVE") && <option>{value}</option>
                          })}
                        </>
                      </select>
                    }
              ...
              <div className="mb-1 ml-4 mr-4 mt-1 mb-2" style={{backgroundColor: '#dfe1e3'}}>
                <div style={{backgroundColor: '#9a9ea3'}}>
                  <Label className="ml-3 mt-1 mb-1 font-weight-bold">Template</Label>
                </div>
                <Col xs="12" md="4" className="row mr-4 ml-4 mt-2 mb-2">
                  <Label className="col-5 text-right font-weight-bold">Template Name:</Label>
                  <Input className="col-7 form-control-sm" type="text" name="template" id="template" value={this.state.template} disabled={this.state.disable} onChange={(ev) => this.handle(ev)}/>
                </Col>
              </div>
              <Row>
                <Col md="6" xs="12">
              ...
              <div className="ml-4 mr-4 mb-2 mt-2">
                <Row>
                  <Col xs="12" md="7">
                    ...
                    <Button size="md" color="primary" className="mr-2" disabled={this.state.update} onClick={this.update_pad}>Update</Button>
                    ...
                    <Button size="md" color="primary" disabled={this.state.delete} onClick={this.toogleDelete}>Delete</Button>
                  </Col>
                  <Col xs="12" md="5" className="text-right">
                    ...
                    <Button size="md" color="danger" className="mr-2" disabled={this.state.rec} onClick={this.toggle_cpr}>REC</Button>
        ...
        {/*Modal for copy pointer record*/}
        <Modal isOpen={this.state.large} toggle={this.toggleLarge} className={'modal-lg ' + this.props.className}>
          <ModalHeader toggle={this.toggleLarge}>Copy Pointer Record</ModalHeader>
          <ModalBody>
            <FormGroup row>
              <Col xs="6">
                <Card>
              ...
              <Col xs="6">
                <Card>
                  ...
                  <CardBody>
                    ...
                    <Label htmlFor="copy_sfed">Effective Date/Time</Label>
                    <Row>
                      <Col xs="7">
                        <DatePicker
          ...
          <ModalFooter>
            <Button type="submit" size="md" color="primary" className="mr-2" onClick={this.copy_pointer}> Copy</Button>
            <Button type="reset" size="md" color="danger" onClick={this.toggleLarge}> Cancel</Button>
          </ModalFooter>
        </Modal>
        {/*Modal for transfer pointer record*/}
        <Modal isOpen={this.state.tran} toggle={this.toggle_transfer} className={'modal-lg ' + this.props.className}>
          <ModalHeader toggle={this.toggle_transfer}>Pointer Record Transfer</ModalHeader>
          <ModalBody>
            <FormGroup row>
              <Col xs="6">
                <Card>
          ...
          <ModalFooter>
            <Button type="submit" size="md" color="primary" className="mr-2" onClick={this.transfer}> Transfer</Button>
            <Button type="reset" size="md" color="danger" onClick={this.toggle_transfer}> Cancel</Button>
          </ModalFooter>
        </Modal>
        {/*Modal for pointer record convert*/}
        <Modal isOpen={this.state.cpr} toggle={this.toggle_cpr} className={'modal-lg ' + this.props.className}>
          <ModalHeader toggle={this.toggle_cpr}>Pointer Record Convert</ModalHeader>
          <ModalBody>
            <FormGroup row>
              <Col xs="6">
                <Card>
                  ...
                  <CardBody>
                    ...
                    <Input type="text" name="ed" id="ed" disabled value={this.state.ed_origin + " " + this.state.et_origin}/>
                  </CardBody>
                </Card>
              </Col>
              <Col xs="6">
          ...
          <ModalFooter>
            <Button type="submit" size="md" color="primary" className="mr-2" onClick={this.convertPadToCad}> Convert PAD to CAD</Button>
            <Button type="reset" size="md" color="danger" onClick={this.toggle_cpr}> Cancel</Button>
          </ModalFooter>
        </Modal>
        {/*Modal for delete pointer record*/}
        <Modal isOpen={this.state.toggle_delete} toggle={this.toogleDelete} className={'modal-lg ' + this.props.className}>
          <ModalHeader toggle={this.toogleDelete}>Delete Pointer Record</ModalHeader>
          <ModalBody>
            <FormGroup row>
              <Col xs="6">
                <Card>
          ...
          <ModalFooter>
            <Button type="submit" size="md" color="primary" className="mr-2" onClick={this.delete_pointer}> Delete</Button>
...
export default connect((state) => ({somos: state.auth.profile.somos}))(withLoadingAndNotification(PointerData));
