...
class TAD extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      copy_now: false, transfer_now: false, delete_now: false, copy_sfed: null, transfer_sfed: null, delete_sfed: null, disconnect_sfed: '', isCopyDate: false, isCopyPortion: false,
      isDeleteDate: false, isTransferDate: false
    };
  ...
  retrieve_template = async (template, sfed) => {
    ...
    if (sfed === "") {
      let message = template_list(this.props.somos.id, this.props.somos.selectRo, "", template);
      let data = [], params = {};
  ...
  update_tad = async (cpr, lad, cr) => {
    ...
    if (this.state.action === "N") {
      ...
      message = copyTad(this.props.somos.id, this.props.somos.selectRo, this.state.action, this.state.copy_template, this.state.sfed, this.state.sfed, this.state.iec, this.state.iac, this.state.notes, this.state.ncon, this.state.ctel, this.state.label, this.state.lata, this.state.network, this.state.state, this.state.line, lads, types, this.state.gridData, cr, cpr, lad, this.state.description);
  ...
  copy_tad = () => {
    if (!this.state.copy_sfed && !this.state.copy_now) {this.setState({isCopyDate: true}); return false;}
  ...
  deleteTad = async () => {
    if (!this.state.delete_sfed && !this.state.delete_now) {this.setState({isDeleteDate: true});return false;}
  ...
  transferTad = async () => {
    if (!this.state.transfer_sfed && !this.state.transfer_now){this.setState({isTransferDate: true}); return false;}
  ...
  copyDate = (date) => {this.setState({copy_sfed: date, isCopyDate: false, isCopyPortion: false});};
  transferDate = (date) => {this.setState({transfer_sfed: date, isTransferDate: false});};
  deleteDate = (date) => {this.setState({delete_sfed: date, isDeleteDate: false});};
  ...
  handleCheck = (event)=> {
    const state = {}; state[event.target.name] = event.target.checked; this.setState(state);
    ...
    return (
      <div className="animated fadeIn mt-1">
              ...
              <Col xs="6">
                <Card>
                  ...
                  <CardBody>
                    ...
                    <Row>
                      <Col xs="7">
                        <DatePicker dateFormat="MM/DD/YY hh:mmA/C" selected={this.state.delete_sfed} showTimeSelect timeIntervals={15} onChange={this.deleteDate} className="form-control"
                                    timeCaption="time"/>
                      </Col>
                      <div className="form-check align-content-center">
        ...
        <Modal isOpen={this.state.isLad} toggle={this.goLad}
            ...
            <Row>
              <Col xs="12">
                <div className="mt-3 mb-1 ml-4 mr-4"
                  ...
                  <Row className="mt-2 ml-4 mr-4 pt-2">
                    <Col xs="12" md="6" className="row">
                ...
                <div className="mb-1 ml-4 mr-4"
                     style={{backgroundColor: '#dfe1e3'}}>
                  <Row className="mb-1 ml-4 mr-4 mt-2 pb-2 pt-2">
                    <Col xs="12" md="3" className="row">
                      <Label className="col-6 font-weight-bold text-right">Resp Org:</Label>
                      <Input className="col-6 form-control-sm" type="text" name="ro" id="ro" onChange={(ev) => this.handle(ev)} value={this.state.ro} disabled={this.state.disable}/>
                    </Col>
                ...
                <Card className="mb-1 ml-4 mr-4 mt-1 mb-1">
                  <div>
                ...
                <div className="ml-4 mr-4 pb-1 pt-1" style={{backgroundColor: '#dfe1e3'}}>
                  <div className="row mt-2 mb-2">
                    <Label className="col-2 text-right">Message:</Label>
                    <Input className="col-8" type="textarea" name="message" id="message" onChange={(ev) => this.handle(ev)} value={this.state.message} disabled={this.state.disable}/>
                  </div>
          ...
          <ModalFooter>
            <Button size="md" color="primary" className="mr-4" onClick={this.goCpr}>CPR</Button>
          </ModalFooter>
        </Modal>
        ...
        <Modal isOpen={this.state.isCpr} toggle={this.goCpr}
          ...
          <ModalBody>
            <Row>
              <Col xs="12">
                ...
                <div className="mb-1 ml-4 mr-4 mt-1 mb-1"
                     style={{backgroundColor: '#dfe1e3'}}>
                  <Col xs="12" md="8" className="row mr-4 ml-4 mt-2 mb-2 pt-2">
                ...
                <div className="ml-4 mr-4 mb-2" style={{backgroundColor: '#dfe1e3'}}>
                  <div className="row mt-2 mb-2 pb-2 pt-2">
                    <Label className="col-2 text-right">Message:</Label>
          ...
          <ModalFooter>
            <Button size="md" color="primary" className="mr-4" onClick={this.goLad}>LAD</Button>
          </ModalFooter>
        </Modal>
      </div>
    );
