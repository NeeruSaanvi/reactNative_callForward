...
import {callApi, callApiHideLoading} from "../../redux/ApiRedux";
...
export default function withLoadingAndNotification(WrappedComponent) {
  ...
  return connect(
    ...
    (dispatch) => ({
      ...
      callApiHideLoading: (method, ...args) => new Promise((resolve, _) => {
        dispatch(callApiHideLoading(method, resolve, ...args))
      }),
